...
CompressionMethod operator++(CompressionMethod& x) {
	return x = (CompressionMethod)(std::underlying_type<CompressionMethod>::type(x) + 1);
}
...
std::string CompressionMethod_toString(const CompressionMethod c) {
	string str;
	switch (c) {
...
std::ostream & operator<<(std::ostream &os, const CompressionMethod c) {
